    ...
    <section class="overflow-hidden bg-white py-11 font-poppins dark:bg-gray-800">
        <div class="max-w-6xl px-4 py-4 mx-auto lg:py-8 md:px-6">
            ...
            <div class="flex flex-wrap -mx-4">
                <div class="w-full mb-8 md:w-1/2 md:mb-0" x-data="{ mainImage: '{{ Storage::url($producto->images[0]) }}' }">
                    <div class="sticky top-0 z-50 overflow-hidden ">
                        ...
                        <div class="flex-wrap hidden md:flex ">
                            ...
                            @foreach ($producto->images as $image)
                                <div class="w-1/2 p-2 sm:w-1/4" x-on:click="mainImage='{{ Storage::url($image) }}'">
                                    <img src="{{ Storage::url($image) }}" alt="{{ $producto->nombre }}"
                    ...
                    <div class="lg:pl-20">
                        ...
                        <div class="flex flex-wrap items-center gap-4">
                            <button wire:click="agregarCarrito({{ $producto->id }})"
                                ...
                                <span wire:loading.remove wire:target='agregarCarrito({{ $producto->id }})'>Agregar a
                                    carrito </span> <span wire:loading
                                    wire:target='agregarCarrito({{ $producto->id }})'>
                                    Agregando...</span></button>
                        </div>
                    </div>
                </div>
            </div>
